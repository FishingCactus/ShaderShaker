...
function cli:add_opt(key, desc, default)
  ...
  assert(type(key) == "string" and type(desc) == "string", "Key and description are mandatory arguments (Strings)")
  assert(type(default) == "string" or default == nil or type( default ) == "boolean", "Default argument: expected a string or nil")
  ...
  -- set defaults
  if v == nil and type( default ) ~= "boolean" then default = false end   -- no value, so its a flag
